...
def get_I_GH(raw_pjoint, n_q, eps=1e-8):
    ...
    for o in pjoint.sample_space():
        for q in range(n_q):
            variables[(q, o)] = Variable(len(variables))  # s(q|x_1,...,x_n,y')
    ...
    for ndx, o in enumerate(pjoint.sample_space()):
        # for each x1, .., x_n, y : \sum_q p(q|x1,...,x_n,y) = 1 
        ...
        for source in pjoint.get_rv_names()[:-1]:
            ...
            for q in range(n_q):
                for xy in pXYoutcomes:
                    ...
                    cs.insert(sumL*normR - sumR*normL == 0)
    ...
    def get_solution_val(x, full=False):
        ...
        mival = mi(pQY)
        return {'raw':np.array(x), 'pQY':pQY}, mival
